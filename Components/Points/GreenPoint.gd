...
const red_attracion: float = StrongForce_Power
const red_attraction_distance: float = StrongForce_Distance
...
func _interact_with_green(other, delta) -> Vector2:
	...
	var speed = (1 / distance) * green_attracion
...
func _interact_with_red(other, delta) -> Vector2:
	...
	var speed: float = 0
	if distance < 5:
		speed = 0#((1 / distance) * red_attracion) * -2
	else:
...
func _interact_with_yellow(other, delta) -> Vector2:
	...
	var speed: float = 0
	if distance < 3:
		speed = 0#((1 / distance) * yellow_attracion) * -2
	else:
